...
<div class="d-flex align-items-center justify-content-between">
    <h3 class="mb-0">Images upload</h3>
...
<div class="container">
    ...
    <div class="modal-content">
        ...
        <div class="row">
            <div class="col-md-12">
                <label>Slide image Status</label>
...
<script>
    ...
    var table = $('#resourcedocuments').DataTable({
      processing: true,
      serverSide: true,
      ajax: "<?php echo e(route('slidesget')); ?>",
      columns: [
    ...
    $("#resourcedocs").validate({
        rules: {
            image : {
                required: true,
                // fileSize: 5120,
                extension: "jpg|png|svg|gif|webp"
            },    
        },
        messages : {
            image: {
                required: "Please upload a file.",
                extension: "You can upload only jpg ,png ,svg ,webp gif extensions files.",
    ...
    function fileUpload(files,token){
      ...
      $.ajax({
        url: "<?php echo e(url('slideimageupload')); ?>",
        method: 'post',
        data:fd,
        contentType: false,
        processData: false,
    ...
    $('.data-table').on('click','.slidedel',function() {
        var id = $(this).attr('id');
    ...
    function resourceFilesDel(id,data){
      ...
      $.ajax({
        method : "GET",
        url: "<?php echo e(route('slidesview')); ?>",
        data : data,
          success: function(response) {
            console.log(response);
    ...
    $('.data-table').on('click','.slideedit',function() { 
        var id = $(this).attr('id');
        var data = $(this).attr('data-id');
    ...
    function resourceFileview(id,data){
        ...
        data : data,
            success: function(response) {
                console.log(response.fileData['active_status']);
                $( "#filedataid" ).empty().val(response.fileData['id']);
                $( "#image_name" ).empty().append("<div><label for='resource_name'>"+response.fileData['image']+"</label></div>");
    ...
    $("#closeModalDel").click(function() {
      ...
      e.preventDefault();
        ...
        $.ajax({
        method : "get",
        url: "<?php echo e(route('destroyslide')); ?>",
        data : data,
          success: function(response) {
            console.log(response);
            $('#myModalDel').modal('hide');
    ...
    function resourceFileUpdate(data){
        var data = data;
        $.ajax({
        url : "<?php echo e(Route('slideupdate')); ?>",
        method : "post",
        data : data,
            success: function(response) {
                $('#myModal').modal('hide');
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\Loreto Sisters\admintemplate\resources\views/slides/index.blade.php ENDPATH**/ ?>