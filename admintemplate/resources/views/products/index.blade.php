...
<script>
 ...
 var table = $('#newseventstable').DataTable({
    ...
    function newsview(id,data){
        ...
        var data = {'id' : id};
        $.ajax({
        url : "{{Route('newsevents.show')}}",
        method : "GET",
        data : data,
            success: function(response) {
                var imageUrl = "{{ asset('new_images/') }}";
